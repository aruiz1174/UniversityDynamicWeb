...
import { getMinor } from "./SharedService";
...
function Minors() {
  ...
  const columns = [{ field: "MajorName", headerName: "Minor Name", width: 400 },
  { field: "DepartmentName", headerName: "Department Name ", width: 300 },
  {
    field: "id", headerName: "Requisites", width: 200, renderCell: (params) => {
      return <a href="./MinorRequirements" onClick={() => { localStorage.setItem("MinorID", params.row.MajorName) }}>Requirements</a>
    }
  }];

  const columnsVisitor = [{ field: "MajorName", headerName: "Minor Name", width: 400 },
  { field: "DepartmentName", headerName: "Department Name ", width: 300 }];
  ...
  function VisitorView(userType, isLoggedIn) {
    ...
    if (showTime(userType.userType, userType.isLoggedIn, userTypes)) {
      return (
        <>
          <h1>All Minor</h1>
          <h4>Here you can view all minors.</h4>
