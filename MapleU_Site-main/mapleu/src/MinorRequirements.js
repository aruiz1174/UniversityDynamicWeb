...
import { getMinorRequirements } from "./SharedService";
...
function MinorRequirements() {
  ...
  useEffect(() => {
    getMinorRequirements().then(x => {
      let MinorReID = localStorage.getItem("MinorID");
      // filter the data based on the courseID
      let filteredData = x.data.filter(x => x.MinorName === MinorReID);
  ...
  return (
    <>
      ...
      <br></br>
      <br></br>
      <h1>Requirements for {tableData[0]?.MinorName}</h1>
